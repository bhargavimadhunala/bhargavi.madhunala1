 ...
 */
    ...
    /**
     * To setup the decryption key schedule.
     */
     ...
     */
    public static final int BOTH    = (ENCRYPT|DECRYPT);
     ...
     * (N.B. the Rijndael algorithm itself allows for other sizes).
     */
    public static final int SIZE  = (BITS >>> 3);
     ...
     */
    private static final int[]
        rcon = new int[10]; /* for 128-bit blocks */
    ...
    public void makeKey(byte[] cipherKey, int keyBits)
            throws RuntimeException {
        makeKey(cipherKey, keyBits, BOTH);
    }

    /**
     * Encrypt exactly one block (BLOCK_SIZE bytes) of plaintext.
     *
     * @param   p          plaintext block.
     * @param   c          ciphertext block.
     */
    public void encrypt(byte[] p, byte[] c) {
        ...
        v = rek[k    ];
        c[ 0] = (byte)(Se[(t0 >>> 24)       ] ^ (v >>> 24));
        c[ 1] = (byte)(Se[(t1 >>> 16) & 0xff] ^ (v >>> 16));
        c[ 2] = (byte)(Se[(t2 >>>  8) & 0xff] ^ (v >>>  8));
        c[ 3] = (byte)(Se[(t3       ) & 0xff] ^ (v       ));
    
        v = rek[k + 1];
        c[ 4] = (byte)(Se[(t1 >>> 24)       ] ^ (v >>> 24));
        c[ 5] = (byte)(Se[(t2 >>> 16) & 0xff] ^ (v >>> 16));
        c[ 6] = (byte)(Se[(t3 >>>  8) & 0xff] ^ (v >>>  8));
        c[ 7] = (byte)(Se[(t0       ) & 0xff] ^ (v       ));
    
        v = rek[k + 2];
        c[ 8] = (byte)(Se[(t2 >>> 24)       ] ^ (v >>> 24));
        c[ 9] = (byte)(Se[(t3 >>> 16) & 0xff] ^ (v >>> 16));
        c[10] = (byte)(Se[(t0 >>>  8) & 0xff] ^ (v >>>  8));
        c[11] = (byte)(Se[(t1       ) & 0xff] ^ (v       ));
    
        v = rek[k + 3];
        c[12] = (byte)(Se[(t3 >>> 24)       ] ^ (v >>> 24));
        c[13] = (byte)(Se[(t0 >>> 16) & 0xff] ^ (v >>> 16));
        c[14] = (byte)(Se[(t1 >>>  8) & 0xff] ^ (v >>>  8));
        c[15] = (byte)(Se[(t2       ) & 0xff] ^ (v       ));
    }
